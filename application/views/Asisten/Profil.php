    ...
    <section class="content">
			<div class="card mt-2">
				<div class="card-header bg-primary">
					<b>Profil</b>
					<select class="custom-select custom-select-sm" id="ListDosen" style="width: 250px;">
						<?php foreach ($ListDosen as $key) { ?>
							<option value="<?=$key['NIP']?>" <?=$key['NIP']==$this->session->userdata('NIP')?'selected':''?>><?=$key['Nama']?></option>
						<?php } ?>
					</select>
					<button class="btn btn-sm btn-danger" id="GantiDosen"><b>GANTI DOSEN</b></button>
				</div>
									...
									<table class="table table-bordered table-striped">
										...
										<tbody>
											...
											<tr style="font-size: 12px;" class="text-center align-middle font-weight-bold">	
												...
												<td class="py-1"><?=$Total==1?'0%':'100%';?></td>
								...
								<div class="row">
									...
									<div class="col-sm-8 mt-2">
										...
										<div class="input-group input-group-sm mb-2">
											...
											<input type="text" class="form-control" value="<?=$Profil['Homebase']?>" disabled>
										</div>
									</div>
									<div class="col-sm-12">
										<div class="input-group input-group-sm mb-2">
          ...
          <div class="modal-body">
            <div class="container">
                  ...
                  <div class="input-group input-group-sm mb-2">
                    ...
                    <input type="text" class="form-control" id="NIP" value="<?=$Profil['NIP']?>" data-inputmask='"mask": "999999999999999999"' data-mask disabled>
                  </div>
                  <div class="input-group input-group-sm mb-2">
                    <div class="input-group-prepend">
                      <span class="input-group-text bg-primary"><b>NIDN</b></span>
                ...
                <div class="col-sm-7">
                  ...
                  <div class="input-group input-group-sm mb-2">
                    ...
                    <select class="custom-select" id="HomebaseDosen">
                      <option value="S1" <?=$Profil['Homebase']=='S1'?'selected':''?>>S1</option>
          ...
          $('#ModalSertifikat').modal("show")
				...
				$("#pak").click(function() {
					if (isNaN($('#DariTahun').val()) || $('#DariTahun').val() == "") {
						...
						window.location = BaseURL + 'Asisten/PAK/'+($('#DariSemester').val()+'-'+$('#HinggaSemester').val())+'/'+($('#DariTahun').val()+'-'+$('#HinggaTahun').val())
						if (document.getElementById("BuktiExcel").checked) {
							var PAK = ['Pendidikan','Penelitian','Pengabdian','Penunjang'] 
							for (let i = 1; i < 5; i++) {
								$.post(BaseURL+"Asisten"+"/Lampiran/"+($('#DariSemester').val()+'-'+$('#HinggaSemester').val())+'/'+($('#DariTahun').val()+'-'+$('#HinggaTahun').val())+'/'+PAK[i-1]).done(function(Respon) {
				...
				$("#bkd").click(function() {
					if (isNaN($('#DariTahun').val()) || $('#DariTahun').val() == "") {
						...
						window.location = BaseURL + 'Asisten/BKD/'+($('#DariSemester').val()+'-'+$('#HinggaSemester').val())+'/'+($('#DariTahun').val()+'-'+$('#HinggaTahun').val())
						if (document.getElementById("BuktiExcel").checked) {
							var PAK = ['Pendidikan','Penelitian','Pengabdian','Penunjang']
							for (let i = 1; i < 5; i++) {
								$.post(BaseURL+"Asisten"+"/LampiranBKD/"+($('#DariSemester').val()+'-'+$('#HinggaSemester').val())+'/'+($('#DariTahun').val()+'-'+$('#HinggaTahun').val())+'/'+PAK[i-1]).done(function(Respon) {
									var array = JSON.parse(Respon)
									var NomorLampiran = 1
									array.forEach(function(object) {
										if (object.Bukti != null) {
				...
				$("#NamaPejabat").click(function() {
					...
					$.post(BaseURL+"Asisten/NamaPejabat", Data).done(function(Respon) {
						if (Respon == '1') {
				...
				$("#GantiDosen").click(function() {
					window.location = BaseURL + "Asisten/GantiDosen/" + $("#ListDosen").val()
				})
				...
				$("#GantiPassword").click(function() {
					if ($("#EditPassword").val() == "") {
						...
						$.post(BaseURL+"Asisten/GantiPassword", Data).done(function(Respon) {
							if (Respon == '1') {
            ...
            alert('Mohon Isi Nama + Gelar!')
						...
						fd.append('Homebase',$("#HomebaseDosen").val())
						...
						$.ajax({
							url: BaseURL+'Asisten/EditProfil',
							type: 'post',
							data: fd,
							contentType: false,
							processData: false,
